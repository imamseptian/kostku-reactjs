...
const MainForm = () => {
  ...
  const [baseFotoKTP, setbaseFotoKTP] = useState("");
  const [baseFotoDiri, setbaseFotoDiri] = useState("");
  ...
  const submitData = () => {
    ...
    axios
      .post(APIUrl + "/api/daftarkost", {
        ...pendaftar,
        foto_ktp: baseFotoKTP,
        foto_diri: baseFotoDiri,
  ...
  return (
    <div className="flex flex-col shadow-md lg:w-4/5 md:w-3/4 sm:w-full mx-auto bg-white rounded-md py-3">
      {/* <div
        className="w-24 h-24 bg-red-800"
        onClick={() => {
          console.log(pendaftar);
          console.log(baseFotoKTP);
        }}
      ></div> */}
        ...
        <div className="w-full">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (e.target.files[0]) {
                setImgKTP({
                  srcKTP: URL.createObjectURL(e.target.files[0]),
                  altKTP: e.target.files[0].name,
                });
              }

              reader.onload = function (upload) {
                // console.log(upload.target.result);
                setbaseFotoKTP(upload.target.result);
                // self.setState({
...
block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4 mb-3`}
        ...
        <div className="my-2">
          <img src={srcDiri} alt={altDiri} className="h-56 w-auto" />
        </div>
